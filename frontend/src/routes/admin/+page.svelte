...
{:else}
	<div class="kpis">
		...
		<div class="kpi">
			<b class="ver">{overview.counts.data_version ?? '—'}</b><span>data_version</span>
		</div>
	</div>
			...
			<div>
				...
				<table>
					<tbody>
						...
						{/each}
						{#if !overview.events_by_type.length}<tr><td class="muted">nenhum evento ainda</td></tr
							>{/if}
					</tbody>
				</table>
			</div>
	...
	{#if tab === 'users'}
		<table>
			<thead
				><tr><th>E-mail</th><th>Nome</th><th>Instituição</th><th>Criado</th><th>Admin</th></tr
				></thead
			>
			<tbody>
				{#each users as u}
