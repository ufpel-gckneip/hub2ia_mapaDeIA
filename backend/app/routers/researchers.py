...
async def list_researchers(
    ...
    if event:
        joins += (
            " JOIN researcher_events re ON re.researcher_id = r.researcher_id"
    ...
    order_sql = {
        "n_articles": "r.n_articles DESC",
        "display_name": "r.display_name ASC",
        "last_year": "r.last_year DESC",
    }[sort]

    total = (
        await session.execute(
            text(
                f"SELECT count(DISTINCT r.researcher_id) "
                f"FROM researchers r{joins} WHERE {where_sql}"
            ),
            params,
        )
    ).scalar_one()
